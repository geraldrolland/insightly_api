...
async def login_user(data: UserLoginType, 
    ...
    if user.is_MFA_enabled:
        ...
        key = str(uuid.uuid4())
        redis_client.set(name=key, value=json.dumps({"otp_code_hash": hash(otp_code), "attempts": 0}), ex=2*60)
        payload = {
            "email": user.email,
            "otp_token": key
        }
...
async def user_email(data: Annotated[EmailType, Body()], session: Annotated[Session, Depends(get_session)]):
    ...
    verification_link = generate_verification_link(user.email, "reset-password")
    body = {"verification_link": verification_link}
    send_verification_email.delay(user.email, body)
...
def logout_user(request: Request, response: Response):
    from insightly_api.main import redis_client
    payload = verify_access_token(verify_signed_cookie(request.cookies.get("auth_token")).get("access_token"))
    redis_client.delete(f'session:{payload.get("sid")}')
