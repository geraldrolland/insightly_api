...
def test_with_expired_access_token_in_auth_token_cookie(client, api_url, user):
    from insightly_api.utils import sign_cookie, generate_access_token, verify_signed_cookie
    import jwt
    from unittest.mock import patch

    access_token, refresh_token = generate_access_token({"email": user.email, "id": user.id, "user-agent": "test-agent"})
...
def test_with_expired_refresh_token_in_auth_token_cookie(client, api_url, session, user):
...
def test_with_incorrect_user_agent_in_request_header(client, api_url):

    client.headers.update({"User-Agent": "incorrect-agent"})
    response = client.get(api_url)
    assert response.status_code == 401