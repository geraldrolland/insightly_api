...
def test_with_empty_payload(client, api_url):
    response = client.put(api_url, json={})
...
def test_with_missing_fields(client, api_url):
    ...
    response = client.put(api_url, json=payload)
    assert response.status_code == 422
def test_with_fake_value_for_field(client, api_url):
    payload = {
        "enable_mfa": "incorrect_value"
    }
    response = client.put(api_url, json=payload)
    assert response.status_code == 422
...
def test_with_enable_mfa_false(client, api_url):
    payload = {
        "enable_mfa": False
    }
    response = client.put(api_url, json=payload)
